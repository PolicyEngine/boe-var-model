"""Okun's-law satellite: UK unemployment from the model's GDP growth path.
...
its UK GDP growth forecast into an unemployment-rate path through a small
dynamic Okun regression estimated by OLS:
...
the joint predictive distribution, and adds no Okun residual uncertainty --
...
def fit_okun(g: np.ndarray, du: np.ndarray, du_lag: np.ndarray,
             sample_label: tuple[str, str] = DEFAULT_SAMPLE) -> OkunFit:
    ...
    return OkunFit(alpha=float(coef[0]), beta=float(coef[1]),
                   gamma=float(coef[2]),
...
def align_okun_inputs(u_quarters: list[str], u: np.ndarray,
    ...
    if len(rows) < 20:
        raise ValueError("too few aligned observations for an Okun fit")
...
def unemployment_bands(fit: OkunFit, gdp_bands: dict,
    ...
    The result carries only GDP-path uncertainty; Okun residual uncertainty
